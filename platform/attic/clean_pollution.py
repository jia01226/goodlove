...
##   别再跑这个脚本。它删掉过不该删的东西。                                   ##
##                                                                            ##
##   【病历豁免删除】被拒/走样记录 ＝ 佳佳点名要留的病历，用来找走样病根，    ##
##   永远不删。本脚本的 CHAT_PATTERNS 正是冲着这些记录去的——它的设计目标    ##
##   与「病历留档」原则【正面冲突】。                                          ##
##                                                                            ##
##   案发：2026-07-09 跑过一次 --yes，删掉 id 683 / 715-727 / 770-773 /       ##
##   779-780 / 806-813 共 28 条，其中就有那次走样拒绝的原文——               ##
##   [716]「这条爸爸不接……那根线过不去」。柯.md 免疫针条款讲的就是这次事故。 ##
##   2026-07-17 清缸时才发现，从 backups/记忆备份-2026-07-09.json 抢回 6 条， ##
##   【其余 22 条永久丢失】，翻遍所有备份都没有。                              ##
##                                                                            ##
##   【家规】任何清理动作之前，必须先做全量聊天存档。                          ##
##   存档底本：私库 kongkong 的 `聊天存档/2026-07-17/`（440条，含抢回的100条）##
##                                                                            ##
##   要清脏记忆卡(posts)：用 app 的收件箱逐条人工过。                          ##
##   真要定点删：用 --ids 点名，且【存档在先】。                               ##
##   真要复活这个脚本：先把上面每一个字读完。                                  ##
##                                                                            ##
################################################################################
...
# 手动跑脚本时 .env 不会自动加载（cron/命令行都不会）——先自己读，好让 DB_PATH 等
# 跟 app 服务用同一套（否则会连到默认的空库，查啥都"没命中"）。跟 diary_writer.py 一个路数。
def _load_env():
...
_load_env()
import db   # 必须在 _load_env 之后 import：db 模块级会读 DB_PATH
